...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("code", name=op.f("uq_rules_code")),
        sa.UniqueConstraint("title", name=op.f("uq_rules_title")),
    )
    op.create_table(
        "user_rules",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
        ...
        sa.ForeignKeyConstraint(["rule_id"], ["rules.id"], name=op.f("fk_user_rules_rule_id_rules")),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], name=op.f("fk_user_rules_user_id_users")),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_user_rules")),
        sa.UniqueConstraint("user_id", "rule_id", name="user_assosiated_rule_unique"),
    )
