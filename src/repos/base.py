...
class BaseRepo(
    ...
    def __handle_integrity_error(
        ...
        if exc.orig and isinstance(
            exc.orig.__cause__, CheckViolationError
        ):
            raise ObjectInvalidValueError from exc
        if exc.orig and isinstance(
            exc.orig.__cause__, ForeignKeyViolationError
        ):
    ...
    async def get_all_filtered(
        ...
        **filter_by,
        ...
        try:
            ...
            if exc.orig and isinstance(
                exc.orig.__cause__, DataError
            ):
                raise ValueOutOfRangeError(
                    detail=exc.orig.__cause__.args[0]
                ) from exc
            raise exc
        return [
    ...
    async def add_bulk(
        self, data: Sequence[SchemaAddType]
    ) -> list[SchemaReturnType]:
        add_obj_stmt = (
            insert(self.model)
    ...
    async def add(
        self, data: SchemaAddType, **params
    ) -> SchemaReturnType:
        add_obj_stmt = (
            insert(self.model)
    ...
    async def get_one_or_add(
        self, data: SchemaAddType, **params
    ) -> tuple[bool, SchemaReturnType]:
        obj = await self.get_one_or_none(
            **data.model_dump()
        )
    ...
    async def delete(
        self, *filter, ensure_existence=True, **filter_by
    ) -> bool:
        if ensure_existence:
            await self.get_one(*filter, **filter_by)
